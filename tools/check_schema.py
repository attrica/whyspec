...
INTENT_SPEC = ROOT / "profiles" / "intent-layer.md"
...
INTENT = ROOT / "schema" / "intent-layer.schema.json"

RULE_RE = re.compile(
    r"^\*\*\[((?:REC|PROV|ENV|VER|INT)-\d{3})\]\*\*(.*?)(?=\n\n|\Z)",
    re.M | re.S,
)
TICKED = re.compile(r"`([^`]+)`")
CORE = ("command", "status")
EXPECTED_CHECKS = 1281
    ...
    for name, schema in schemas.items():
        for path, node in walk(schema):
            ...
            if parent == "properties" and "description" in node:
                ...
                cited = re.findall(r"(?:REC|PROV|ENV|VER|INT)-\d{3}", node.get("description", ""))
                rep.check(
                    bool(cited),
